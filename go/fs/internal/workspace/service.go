...
// It spawns a sandbox, commits the files, pushes it to the main repository,
// closes the sandbox, and instantly previews the candidate for human review.
func (s *Service) ProposeCandidate(ctx context.Context, thread *Thread, toolName string, files map[string][]byte) (*Candidate, error) {
	...
	sandboxDir, err := s.state.SpawnSandbox(ctx, s.workspaceRoot, thread.ID, candidateID)
	if err != nil {
		return nil, fmt.Errorf("spawning sandbox: %w", err)
	}

	// Ensure cleanup of the physical sandbox even if subsequent steps fail
	defer s.state.CloseSandbox(ctx, sandboxDir)

	// 2. Write tool outputs into the isolated sandbox
	var fileList []string
	for relativePath, content := range files {
		cleanPath := filepath.Clean(relativePath)
		absolutePath := filepath.Join(sandboxDir, "chats", thread.ID, "docs", cleanPath)

		if err := os.MkdirAll(filepath.Dir(absolutePath), 0755); err != nil {
			return nil, fmt.Errorf("creating dir for %s: %w", cleanPath, err)
		}
		if err := os.WriteFile(absolutePath, content, 0644); err != nil {
			return nil, fmt.Errorf("writing file %s: %w", cleanPath, err)
		}
	...
	commitHash, err := s.state.CommitSandbox(ctx, sandboxDir, commitMessage)
	if err != nil {
		return nil, fmt.Errorf("committing sandbox: %w", err)
	}

	// 4. Submit back to main thread context
	if err := s.state.SubmitSandbox(ctx, sandboxDir, s.workspaceRoot, candidateID); err != nil {
		return nil, fmt.Errorf("submitting sandbox: %w", err)
	}

	// 5. Preview the candidate locally for the user
	if err := s.state.PreviewCandidate(ctx, s.workspaceRoot, thread.ID, candidateID); err != nil {
