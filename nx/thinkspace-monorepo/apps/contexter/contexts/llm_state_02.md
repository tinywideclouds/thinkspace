...
describe('ChatStateService', () => {
  ...
  it('should send prompt and append user and system message to coreChat', () => {
    ...
    expect(chat[1].source).toBe('system');
    expect(chat[1].content).toContain('thinking');
  });
  ...
  it('should fetch flow receipt XML over HTTP', async () => {
    service.activeChatId.set('test-chat');
    
    const fetchPromise = service.inspectFlow('flow-999');
    
    const req = httpTestingController.expectOne('/api/receipts/test-chat/flow-999');
    expect(req.request.method).toBe('GET');
    
    req.flush('<FlowReceipt FlowID="flow-999"></FlowReceipt>');
...
export interface ChatItem {
  ...
  content: string;
  flowId?: string; // Used to link to the flow receipt
}
...
export class ChatStateService {
  ...
  public activeChatId = signal<string>('default-chat');
  ...
  // Tracks the currently viewed XML receipt for the split-pane inspector
  ...
  public async inspectFlow(flowId: string): Promise<void> {
    try {
      const url = `/api/receipts/${encodeURIComponent(this.activeChatId())}/${encodeURIComponent(flowId)}`;
      const xml = await firstValueFrom(this.http.get(url, { responseType: 'text' }));
      this.inspectedReceipt.set(xml);
    } catch (err) {
      console.error(`Failed to fetch receipt for flow ${flowId}`, err);
    ...
    switch (event.type) {
      ...
      case 'agent_start':
        this.activeAgents.update(map => {
          ...
          newMap.set(event.agentId, { id: event.agentId, instructions: event.instructions, stream: '', status: 'running' });
          return newMap;
        });
        break;
  ...
  private handleFlowEvent(event: DomainFlowEvent): void {
    this.flowStates.update(flows => {
      const newMap = new Map(flows);
      let flow = newMap.get(event.flowId);
      ...
      if (event.eventType === 'flow_complete' && flow) {
        ...
        if (flow.completedCount === flow.agentCount && flow.status !== 'completed') {
          ...
          this.coreChat.update(chat => [...chat, { 
