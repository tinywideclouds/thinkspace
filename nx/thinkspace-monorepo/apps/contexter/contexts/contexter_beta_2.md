...
export class OutputOverlayComponent {
  ...
  // Idiomatic Angular Signals usage for derived state
              ...
              <div class="flex items-start justify-between mt-1 min-h-[20px]">
                ...
                @if (canSaveAsNew) {
                  <button 
                    ...
                    Save as new ({{ incrementedFilename() }})
                  </button>
                }
              </div>
            </div>
...
export class SelectionModalComponent {
  ...
  constructor() {
    // Replaces ngOnChanges for syncing the default name when opened
    effect(() => {
  ...
  get incrementedFilename(): string {
    const def = this.defaultFilename();
    if (!def) return '';
    const match = def.match(/^(.*?)_?(\d*)(\.[a-zA-Z0-9]+)$/);
    if (!match) return def;
