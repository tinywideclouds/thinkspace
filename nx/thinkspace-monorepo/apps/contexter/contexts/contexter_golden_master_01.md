...
<ul class="list-none pl-4 border-l border-gray-200">
  @for (node of nodes(); track node.path) {
    <li class="my-1">
      @if (node.isDirectory) {
        <div 
          class="cursor-pointer font-medium text-gray-700 flex items-center gap-2 p-1.5 rounded hover:bg-blue-50 hover:text-blue-700 transition-colors select-none group"
          ...
          <!-- Quick Focus Button (Visible on Hover) -->
          <button 
            (click)="$event.stopPropagation(); setRoot.emit(node.path)"
            class="opacity-0 group-hover:opacity-100 ml-auto mr-2 text-[10px] font-bold bg-blue-100 hover:bg-blue-200 px-1.5 py-0.5 rounded text-blue-700 transition-opacity"
...
export class FileTreeComponent {
  ...
  selectedFiles = input<Set<string>>(new Set());
  expandedNodes = input<Set<string>>(new Set());
  
  fileToggled = output<string>();
  folderToggled = output<FileNode>();
  directoryToggled = output<FileNode>();
  expandToggled = output<FileNode>();
  setRoot = output<string>();
