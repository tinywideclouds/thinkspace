...
export class OutputOverlayComponent {
  ...
  onFilenameFocus() {
    if (!this.userHasEditedFilename && this.customFilename === this.suggestedNewFilename) {
      this.customFilename = '';
    }
    this.userHasEditedFilename = true;
  }
  ...
  useSuggestedName() {
    this.customFilename = this.suggestedNewFilename;
    this.userHasEditedFilename = true;
  }
  ...
  willOverwrite(): boolean {
    ...
    let finalName = name;
    if (finalName && !finalName.endsWith('.md')) {
      finalName += '.md';
    }
...
<div class="flex flex-col h-full bg-gray-50">
  <div class="p-4 border-b border-gray-200 bg-gray-100 flex justify-between items-center">
    <div class="flex items-center gap-3">
      ...
      @if (currentFilename) {
        <span 
          class="text-[10px] font-bold text-blue-700 bg-blue-100 px-2 py-0.5 rounded border border-blue-200 flex items-center"
          [title]="hasUnsavedChanges ? 'You have unsaved changes' : 'Currently loaded selection'">
          {{ currentFilename }}
          @if (hasUnsavedChanges) {
            <span class="text-orange-500 text-sm leading-none ml-0.5">*</span>
          }
        </span>
      }
    </div>
          ...
          <div class="flex gap-3">
            <div class="flex-1">
              <div class="flex justify-between items-end mb-1">
                <label class="block text-xs text-gray-600">Filename</label>
                @if (suggestedNewFilename && customFilename !== suggestedNewFilename) {
                  <button 
                    (click)="useSuggestedName()" 
                    class="text-[10px] text-blue-600 hover:text-blue-800 hover:underline font-bold transition-colors">
                    Save as new ({{ suggestedNewFilename }})
                  </button>
                }
              </div>
              <input 
  ...
  async onSaveContext(payload: SaveContextPayload) {
    ...
    try {
      ...
      if (response?.success) {
        ...
        const activeSelection = this.selection.currentSelectionFilename();
        if (activeSelection) {
          await this.selection.saveSelection(
            activeSelection, 
...
export class SelectionModalComponent {
  ...
  onFocus() {
    // Only clear the input if it's the generic auto-generated name.
    // If it's a loaded selection name, leave it alone so it can be edited.
    if (!this.userEdited && this.customFilename === this.suggestedNewFilename) {
      this.customFilename = '';
    }
    this.userEdited = true;
  }
  ...
  willOverwrite(): boolean {
    ...
    let finalName = name;
    if (finalName && !finalName.endsWith('.yaml')) {
      finalName += '.yaml';
    }
    return this.existingFiles.includes(finalName);
  }
    ...
    <div class="bg-white rounded-lg shadow-xl w-full max-w-lg overflow-hidden flex flex-col">
      ...
      @if (mode === 'save') {
        <div class="p-4 flex flex-col gap-4">
          <div>
            <div class="flex justify-between items-end mb-1">
              <label class="block text-xs font-bold text-gray-700">Filename</label>
              @if (suggestedNewFilename && customFilename !== suggestedNewFilename) {
                <button 
                  (click)="useSuggestedName()" 
                  class="text-[10px] text-blue-600 hover:text-blue-800 hover:underline font-bold transition-colors">
