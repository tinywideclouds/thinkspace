  ...
  <div class="flex flex-col gap-3">
    ...
    <div class="flex gap-2">
      <button 
        ...
        [disabled]="selectedCount === 0"
        class="flex-1 bg-gray-100 hover:bg-gray-200 disabled:bg-gray-50 disabled:text-gray-400 text-gray-700 font-medium py-2 px-2 rounded transition-colors text-sm">
        Save
      </button>
      
      <button 
        (click)="loadSelection.emit('temp_placeholder')"
        class="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium py-2 px-2 rounded transition-colors text-sm">
        Load
      </button>
      ...
      <button 
        ...
        [disabled]="selectedCount === 0"
        class="flex-1 bg-red-50 hover:bg-red-100 disabled:bg-gray-50 disabled:text-gray-400 text-red-600 font-medium py-2 px-2 rounded transition-colors text-sm">
        Clear
      </button>
    </div>
...
<div class="h-screen w-full flex overflow-hidden bg-white text-gray-900 font-sans relative">
  ...
  <main class="flex-1 flex flex-col min-w-0 border-r border-gray-200 relative">
    ...
    <lib-output-overlay
      ...
      [defaultFilename]="bundle.nextContextFilename()"
    ...
    <lib-selection-modal
      ...
      [defaultFilename]="bundle.nextSelectionFilename()"
...
export class LayoutComponent {
  ...
  constructor() {
    this.workspace.loadConfig().then(conf => {
      // Auto-load files from config if defined
  ...
  async onSaveSelection(payload: {filename: string}) {
    try {
      const response = await this.selection.saveSelection(payload.filename);
      if (response?.success) {
...
<div class="flex flex-col h-full bg-gray-50">
  <div class="p-4 border-b border-gray-200 bg-gray-100 flex justify-between items-center">
    ...
    @if (missingFiles.size > 0) {
      <span class="text-red-500 font-bold cursor-help" title="Some files in your selection are no longer available on disk. They will be skipped during generation.">[!]</span>
    }
  </div>
...
### `libs\contexter\data-access\src\lib\contexter.service.ts`
```
import { inject, Injectable, signal } from '@angular/core';
import { HttpClient } from '@angular/common/http';
...
export class ContexterService {
  ...
  private findNode(nodes: FileNode[], targetPath: string): FileNode | undefined {
  ...
  async saveSelection(filename?: string, description?: string) {
    const files = Array.from(this.selectedFiles());
    ...
    if (res.success) await this.loadDefaults();
    return res;
  }
  ...
  async loadSelection(filename: string) {
    try {
      const payload = await firstValueFrom(
        this.http.get<{files: string[]}>(`${this.baseUrl}/selections/${filename}`)
      );
      ...
      for (const dir of sortedDirs) {
        ...
        const node = this.findNode(this.fileTree(), dir);
        if (node && !node.children) {
          try {
            const children = await firstValueFrom(
              this.http.get<FileNode[]>(`${this.baseUrl}/tree?dir=${encodeURIComponent(dir)}`)
            );
            this.fileTree.update(tree => this.updateTree(tree, dir, this.sortNodes(children)));
          } catch (e) {
      ...
      for (const dir of sortedDirs) {
        ...
        if (node && !node.children) {
          try {
            ...
            // Sort new children and update tree via WorkspaceService
...
apiRouter.get('/selections/:filename', async (req, res) => {
  try {
    ...
    const parsed = yaml.load(content) as { files: string[] };
    res.json(parsed);
  } catch (error) {
...
apiRouter.post('/selections', async (req, res) => {
  try {
    const { files, filename: customFilename, description } = req.body;
    const config = await getConfig();
    ...
    const data = description ? { description, files } : { files };
...
Run `nx test contexter-ui` to execute the unit tests.
