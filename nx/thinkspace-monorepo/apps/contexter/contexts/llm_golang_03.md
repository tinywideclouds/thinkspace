...
const file_src_thinkspace_api_v1_events_proto_rawDesc = "" +
	...
	" \x01(\v2).thinkspace.api.v1.AvailableSpacesPayloadH\x00R\x0favailableSpaces\x12D\n" +
	"\n" +
...
func main() {
	...
	baseRoot := filepath.Join(homeDir, "Documents", *rootName)
	configsDir := filepath.Join(baseRoot, "configs")

	// Ensure base directories and the CLI-requested default space exist physically
	_ = os.MkdirAll(configsDir, 0755)
	_ = os.MkdirAll(filepath.Join(baseRoot, *spaceName), 0755)

	factory := func(repoRoot string) workspace.ChatEngine {
		if *engineType == "exec" {
			return gitfs.NewGoExecChat(repoRoot, rootMode)
		}
		return gitfs.NewGoGitChat(repoRoot, rootMode)
	}
	...
	// Clean Bootstrap: Ensure all physical directories have an in-memory configuration
	physicalSpaces := serviceManager.ListSpaces()
	for _, spaceID := range physicalSpaces {
		if spaceID == "configs" {
			continue // System folder exclusion
		}

		if _, exists := registry.GetSpace(spaceID); !exists {
			logger.Info("Bootstrapping unconfigured physical space into memory", "space", spaceID)
			registry.RegisterSpace(spaceID, &config.DefaultSpace{ID: spaceID})
		}
	}
	...
	appCfg := workspace.AppConfig{DefaultSpace: *spaceName, DefaultChat: *chatName}
	mgmtAPI := api.NewManagementAPI(registry, serviceManager, appCfg)
	mgmtAPI.RegisterHandlers(mux)

	wsServer := api.NewServer(logger, registry, serviceManager, llmMgr, fanOutFlow, modelClient)
...
### `..\..\go\fs\config\thinkspace\fanout.yaml`
```
fanout:
  type: "flow"
  description: "Delegate a coding task to parallel sub-agents."
  retry_prompt: |
    Verification failed. Output trace:
    {{.ErrorTrace}}
```


### `..\..\go\fs\config\thinkspace\golang.yaml`
```
name: "golang"
models:
...
system_prompt: |
  You are operating within a Golang ThinkSpace as a Lead Software Architect. 
...
base_agent_rules: |
  ...
  You must output valid, compiling, idiomatic Go (1.27 or higher) code. 
  ...
  - All Go source code and your `go.mod` file MUST be placed inside a `src/` directory (e.g., `"src/go.mod"`, `"src/circle/circle.go"`).
...
// setupTestServer returns a registered mux and the expected space ID for downstream tests.
...
`
	...
	// Dummy factory since tests rely on mock clients, not git operations here
	...
	appCfg := workspace.AppConfig{DefaultSpace: "golang", DefaultChat: "test-chat"}
	mgmtAPI := api.NewManagementAPI(registry, manager, appCfg)
	mgmtAPI.RegisterHandlers(mux)

	wsServer := api.NewServer(logger, registry, manager, llmMgr, flow, mockClient)
	wsServer.RegisterHandlers(mux)

	return mux, "golang"
}

// TestServer_HandleGetSpaces logic was successfully migrated to ManagementAPI.
// We test a 404 on the old route to ensure clean deprecation, as GET /api/spaces is now on the mux.
func TestServer_LegacyRoutes(t *testing.T) {
	mux, _ := setupTestServer(t)

	// Verify the new endpoint works as expected on the mux
...
func (s *Server) handleWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	// Initial handshake payload uses the same struct defined in management.go
	var availableSpaces []SpaceInfo
	for _, id := range s.manager.ListSpaces() {
		name := id
		isConfigured := false

		if spaceCfg, ok := s.registry.GetSpace(id); ok {
			name = spaceCfg.Name()
			if _, isDefault := spaceCfg.(*config.DefaultSpace); !isDefault {
				isConfigured = true
			}
		}

		availableSpaces = append(availableSpaces, SpaceInfo{
			ID:           id,
			Name:         name,
	...
	for {
		...
		case "submit_prompt":
			...
			// The bootstrapper guarantees this space exists in memory
			activeSpace, _ := s.registry.GetSpace(payload.SpaceID)

			baseRules := "You are a helpful software engineering assistant."
			if spaceConfig, ok := s.registry.GetConfig(payload.SpaceID); ok {
				baseRules = spaceConfig.BaseAgentRules
			}
			...
			go func(promptText string, space workspace.ThinkSpace, spaceID string, chatID string, rules string, cfg flows.FlowConfig) {
				service := s.manager.GetService(spaceID)

				thread, err := service.StartThread(wsCtx, chatID)
				if err != nil {
					s.logger.Error("Failed to start thread", "error", err)
					return
				}

				_ = service.LogUserPrompt(wsCtx, thread, promptText)
				...
				if err := coordinator.ExecuteTurn(wsCtx, thread, space, history, ui); err != nil {
					s.logger.Error("Coordinator execution failed", "error", err)
					ui.OnTextChunk(fmt.Sprintf("\n\n⚠️ System Error: %v\n", err))
				}
			}(payload.Text, activeSpace, payload.SpaceID, payload.ChatID, baseRules, flowCfg)
...
func TestServer_WebSocketHandshake(t *testing.T) {
	mux, expectedSpaceID := setupTestServer(t)
...
type SpaceConfig struct {
	...
	NameStr        string            `yaml:"name"` // Mapped to NameStr to avoid conflict with Name() interface method
	BaseAgentRules string            `yaml:"base_agent_rules"`
	Models         map[string]string `yaml:"models"`
}

func (c *SpaceConfig) Name() string {
	return c.NameStr
}

func (c *SpaceConfig) Model(category workspace.ModelCategory) string {
	if m, ok := c.Models[string(category)]; ok {
		return m
	}
...
// DefaultSpace provides a clean in-memory fallback for directories without YAML configs.
type DefaultSpace struct {
	ID string
}

func (d *DefaultSpace) Name() string                                  { return d.ID }
func (d *DefaultSpace) Model(category workspace.ModelCategory) string { return "gemini-1.5-pro" }
...
// RegisterSpace allows injecting a space directly into memory during bootstrap.
func (r *Registry) RegisterSpace(id string, space workspace.ThinkSpace) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.spaces[id] = space
}
...
func (r *Registry) LoadFS(fsys fs.FS, dir string) error {
	...
	for _, entry := range entries {
		...
		for key, value := range topLevel {
			nodeData, err := yaml.Marshal(value)
			if err != nil {
				continue
			}
			...
			if err := yaml.Unmarshal(nodeData, &typeCheck); err != nil {
				continue
			}

			if typeCheck.Type == "space" {
				var cfg SpaceConfig
				if err := yaml.Unmarshal(nodeData, &cfg); err == nil {
					r.configs[key] = &cfg
					r.spaces[key] = &cfg
				}
			} else if typeCheck.Type == "flow" {
				var cfg FlowConfig
				if err := yaml.Unmarshal(nodeData, &cfg); err == nil {
					r.flows[key] = cfg
				}
...
func (r *Registry) GetConfig(id string) (*SpaceConfig, bool) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	cfg, ok := r.configs[id]
	return cfg, ok
...
func (r *Registry) GetFlow(id string) (FlowConfig, bool) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	flow, ok := r.flows[id]
	return flow, ok
...
func (r *Registry) GetAllConfigs() map[string]*SpaceConfig {
	r.mu.RLock()
	defer r.mu.RUnlock()
	res := make(map[string]*SpaceConfig)
...
func (r *Registry) GetAllFlows() map[string]FlowConfig {
	r.mu.RLock()
	defer r.mu.RUnlock()
	res := make(map[string]FlowConfig)
...
func (r *Registry) GetAvailableSpaces() []SpaceInfo {
	r.mu.RLock()
	defer r.mu.RUnlock()
	var spaces []SpaceInfo
	for k, v := range r.configs {
		spaces = append(spaces, SpaceInfo{ID: k, Name: v.Name()})
	}
	return spaces
}
...
### `..\..\go\fs\internal\llm\manager_test.go`
```
package llm_test
...
### `..\..\go\fs\internal\workspace\context_assembler.go`
```
package workspace
...
### `..\..\go\fs\internal\workspace\event_test.go`
```
...
### `..\..\go\fs\internal\workspace\event.go`
```
package workspace
...
// AppConfig holds the startup defaults for the frontend to bootstrap its URL.
...
func TestManagementAPI_GetConfig(t *testing.T) {
	...
	if cfg.DefaultSpace != "golang" {
		t.Errorf("expected default space 'golang', got '%s'", cfg.DefaultSpace)
	}
}
...
func TestManagementAPI_CreateAndListChats(t *testing.T) {
	...
	var createdChat workspace.ChatMeta
	if err := json.NewDecoder(rr.Body).Decode(&createdChat); err != nil {
		t.Fatalf("failed to decode created chat: %v", err)
	}
	if createdChat.Name != "REST Chat" {
		t.Errorf("expected chat name 'REST Chat', got '%s'", createdChat.Name)
	}
	...
	var chats []workspace.ChatMeta
	if err := json.NewDecoder(rrList.Body).Decode(&chats); err != nil {
		t.Fatalf("failed to decode chat list: %v", err)
	}
	if len(chats) != 1 {
		t.Fatalf("expected 1 chat, got %d", len(chats))
	}
	if chats[0].ID != createdChat.ID {
...
// RegisterHandlers attaches the management REST endpoints to the provided multiplexer.
...
func (api *ManagementAPI) handleGetConfig(w http.ResponseWriter, r *http.Request) {
	...
	_ = json.NewEncoder(w).Encode(api.appCfg)
}

func (api *ManagementAPI) handleGetSpaces(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	_ = json.NewEncoder(w).Encode(api.registry.GetAllConfigs())
}
...
`),
	...
	// 6. Test RegisterSpace (Clean Bootstrap fallback)
	registry.RegisterSpace("unconfigured-space", &config.DefaultSpace{ID: "unconfigured-space"})

	unconfigured, ok := registry.GetSpace("unconfigured-space")
	if !ok {
		t.Fatalf("GetSpace: Expected to find 'unconfigured-space' after registration")
	}

	if unconfigured.Model(workspace.ModelCategoryWorker) != "gemini-1.5-pro" {
		t.Errorf("Expected fallback model 'gemini-1.5-pro', got %q", unconfigured.Model(workspace.ModelCategoryWorker))
	}
}
...
// EngineFactory defines how to create a ChatEngine for a specific path.
type EngineFactory func(repoRoot string) ChatEngine

// ServiceManager lazily instantiates services and manages static directory CRUD operations.
...
// GetService returns an existing Service for the requested space, or initializes a new one.
...
// ListChats scans the space's chats directory and returns sorted metadata.
...
// CreateChat generates a new stable chat directory with an auto-incrementing name if omitted.
